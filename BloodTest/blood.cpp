...
int main(int argc, char **argv)
{
    ...
    for (const auto &sample : getAllSampleTypes()) {
        ...
        std::cout << sampleTypeName(sample) << ": "<< "Mean intensity = " << intensity << ", "<< "Estimated concentration = " << getConcentrationValue(lineCoef, intensity)<< std::endl;
    }
    ...
    const auto printVariation = [&lineCoef, &meanIntensityMap] (SampleType sampleType) {
        ...
        std::cout << "Variation " << sampleTypeName(sampleType) << " = "<< std::fabs(actualConc - refConc) / refConc * 100 << "%"<< std::endl;
    };
